...
namespace Orcs_Plus
{
    public class Ch_H_Orcs_MadnessFestival : ChallengeHoly
    {
        ...
        {
            if (location.soc is SG_Orc orcSociety && ModCore.Get().data.orcSGCultureMap.TryGetValue(orcSociety, out HolyOrder_Orcs orcCulture) && orcCulture != null && orcCulture.tenet_god is H_Orcs_HarbingersMadness harbringer && harbringer.status < 0)
            {
                foreach (Location neighbour in location.getNeighbours())
                {
