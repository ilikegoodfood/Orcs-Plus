...
namespace Orcs_Plus
{
    public class Ch_RecoverShipwreck : Challenge
    {
        ...
        public Ch_RecoverShipwreck(Location location, Pr_Shipwreck wreck)
            : base(location)
        {
            this.wreck = wreck;
        }
        ...
        {
            return "Costs " + cost + " gold, requires the shipwreck to have a minimum charge of " + minCharge + ", and an orc horde to have an empty shipyard.";
        }
        ...
        public override void complete(UA u)
        {
            ...
            if (emptyShipyards.Count > 0)
            {
                ...
                msgString = "The orc shipyard at " + emptyShipyard.location.shortName + " has received the ships";
                location.properties.Remove(wreck);

                if (u.isCommandable() && emptyShipyard.location.soc is SG_Orc orcSociety && ModCore.core.data.orcSGCultureMap.TryGetValue(orcSociety, out HolyOrder_Orcs orcCulture) && orcCulture != null)
                {
                    ModCore.core.TryAddInfluenceGain(orcCulture, new ReasonMsg(getName(), ModCore.core.data.influenceGain[ModData.influenceGainAction.BuildShipyard]), true);
                }
            }
        }
